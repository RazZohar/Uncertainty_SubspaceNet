#!/usr/bin/env python3
"""Evaluate pretrained SubspaceNet, TransMUSIC, DataDrivenComplex and ESPRIT.
One common native IQ realization set per condition; no training. Export combined
and per-model CSVs, per-trial CSV/NPZ, comparison figures, and matching PGFPlots.
Run this script from the project root containing src/.
"""
import argparse
import hashlib
import json
...
def parse_args(argv=None):
    ...
    p.add_argument('--model_types',nargs='+',choices=['multi_subarray','transmusic','data_driven_complex'],default=['multi_subarray','transmusic','data_driven_complex'])
    p.add_argument('--pretrained_root',type=Path,default=Path('sweeps'))
    p.add_argument('--checkpoint_name',default='best_base_reliability.pth')
    p.add_argument('--models_json',type=Path,help='Optional explicit list of model_type, checkpoint_path, config_path, and label entries')
    p.add_argument('--simulation_config',type=Path,help='Explicit common evaluation config if training simulator settings differ')
    p.add_argument('--results_dir',type=Path,default=Path('sweep_results/inter_source_comparison'))
    p.add_argument('--include_esprit',action=argparse.BooleanOptionalAction,default=True)
    p.add_argument('--trial_csv',action=argparse.BooleanOptionalAction,default=True)
...
def main(argv=None):
    ...
    if args.plots_only:
        from inter_source_tools.comparison import export_all
        export_all(args.results_dir.resolve());return
    ...
    targets,cfg=resolve_targets(args)
    ...
    plan=dict(mode='pretrained_comparison',configuration=cfg,targets=targets,cases=cases,case_count=len(cases),
    ...
    for target in targets:print(target['label'],target['checkpoint'],flush=True)
    ...
    from inter_source_tools.comparison import export_all,write_trials,save_summaries
    import torch
    device=('cuda' if torch.cuda.is_available() else 'cpu') if args.device=='auto' else args.device
    source_files=[ROOT/'run_source_sweep_comparison.py',*sorted((ROOT/'inter_source_tools').glob('*.py')),*sorted((ROOT/'src').rglob('*.py'))]
    sources={str(p.relative_to(ROOT)):digest(p) for p in source_files if p.exists()}
    signature=dict(configuration=cfg,targets=targets,cases=cases,trials=args.trials,batch_size=args.batch_size,
        ...
        ellipse_moving_values=args.ellipse_moving_values,trial_csv=args.trial_csv)
    ...
    predictors={t['label']:NativePredictor(t,t['configuration'],Path(t['config_path']) if t['config_path'] else None,args,device) for t in targets}
    wandb_run=None
