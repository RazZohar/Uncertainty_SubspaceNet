# ---------------- trainer.py ----------------
"""
Strict-CP compatible trainer revised: Supports multi-stage training curricula via a JSON configuration.
...
def unwrap_subset_dataset(dataset):
    """Return the underlying dataset for a (possibly nested) torch Subset."""
...
class Trainer:
    def __init__(self, args):
        ...
        # ---------------- Dataset ----------------
        # This file can be a full SensorSourceGraphDataset or a saved Subset of it.
        # In the strict-CP workflow, it is the fixed 80% development split and
        # the remaining 20% is stored separately for conformal calibration.
        full_ds = torch.load(args.dataset_path, weights_only=False)
        base_ds = unwrap_subset_dataset(full_ds)
        if hasattr(base_ds, "set_use_graph_features"):
            base_ds.set_use_graph_features(use_features=True)

        if not (0.0 < args.val_split < 1.0):
            raise ValueError(
                "val_split must be strictly between 0 and 1. In the strict-CP "
                "workflow it must be drawn only from the supplied development split."
            )
        val_len = int(len(full_ds) * args.val_split)
        val_len = max(1, val_len)
        train_len = len(full_ds) - val_len
        if train_len < 1:
            raise ValueError("The supplied development split is too small after validation split.")
    ...
    def _extract_sensor_positions(self):
        sensor_positions = unwrap_subset_dataset(self.train_ds.dataset).get_sensor_potision()
