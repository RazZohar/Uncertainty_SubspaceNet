...
try:
    from sweeps_scripts.compare_datadriven_vs_multisubarray_covariance import (
        PredictionSet,
        covariance_metrics,
    ...
    raise ImportError(
        "Could not import compare_datadriven_vs_multisubarray_covariance.py. "
...
def train_model_if_needed(
    *,
    ...
    if args.reuse_existing and os.path.isfile(latest_ckpt):
        print(f"Reusing existing checkpoint for {model_type}: {latest_ckpt}")
        return latest_ckpt
    ...
    if not os.path.isfile(latest_ckpt):
        # Some staged trainers save best_*.pth but not latest if interrupted; fail clearly.
        raise FileNotFoundError(f"Training finished but checkpoint was not found: {latest_ckpt}")
    return latest_ckpt
...
def compare_one_snr(
    *,
    ...
    print("  [5/5] MultiSubarray covariance on test set")
    ...
    for snr_str in args.snr_values:
        ...
        try:
            ...
            if args.skip_training:
                dd_ckpt = os.path.join(data_driven_dir, "latest.pth")
                ms_ckpt = os.path.join(multi_subarray_dir, "latest.pth")
                if not os.path.isfile(dd_ckpt) or not os.path.isfile(ms_ckpt):
                    raise FileNotFoundError(
                        "--skip_training was set but one or both checkpoints are missing: "
                        f"{dd_ckpt}, {ms_ckpt}"
                    )
            else:
