#!/usr/bin/env python3
"""
sweep_snr_ssn_stage1_baselines_vs_stage2_existing.py

Run the requested covariance/UQ comparison across an existing SNR sweep.
No training and no dataset generation are performed.

Both the Stage-1 MC-Dropout and Stage-1 Bayesian MC-Dropout baselines use
only diagonal covariance diag(Var_w[theta_hat_w]).

Baseline methods use SubspaceNet Stage 1 / base accuracy checkpoint:
    best_base_accuracy.pth

Proposed method uses SubspaceNet Stage 2 / base reliability checkpoint:
    best_base_reliability.pth

Expected default layout per SNR value:
  <sweep_root>/<run_prefix>_<snr>/multi_subarray/best_base_accuracy.pth
  <sweep_root>/<run_prefix>_<snr>/multi_subarray/best_base_reliability.pth
  <sweep_root>/<run_prefix>_<snr>/dataset.pt
  <sweep_root>/<run_prefix>_<snr>/test_dataset.pt
  <sweep_root>/<run_prefix>_<snr>/config.json

You may also provide separate roots for checkpoints, datasets, and configs.
"""
...
from sweeps_scripts.compare_ssn_stage1_baselines_vs_stage2_diagonal_bayesian import main as compare_main
...
def main(argv: Optional[List[str]] = None) -> List[Dict[str, Any]]:
    parser = argparse.ArgumentParser(
        "Evaluate Stage-1 UQ baselines vs Stage-2 proposed covariance over an existing SNR sweep"
    )
    ...
    parser.add_argument("--cal_dataset_name", default="dataset.pt")
    ...
    for snr_text in args.snr_values:
        ...
        cal_dataset_path = require_file(
            os.path.join(dataset_run, args.cal_dataset_name),
            "calibration dataset",
        )
        ...
        print(f"Stage 2 checkpoint: {stage2_checkpoint_path}")
        print(f"Calibration data: {cal_dataset_path}")
        ...
        for row in rows:
            ...
            enriched["cal_dataset_path"] = cal_dataset_path
            enriched["test_dataset_path"] = test_dataset_path
            all_rows.append(enriched)

        write_csv(
            os.path.join(snr_out, "ssn_stage1_baselines_vs_stage2_with_snr.csv"),
            [dict(row, snr_db=snr_value) for row in rows],
        )

    combined_csv = os.path.join(out_root, "ssn_stage1_baselines_vs_stage2_snr_all.csv")
    combined_json = os.path.join(out_root, "ssn_stage1_baselines_vs_stage2_snr_all.json")
    combined_pt = os.path.join(out_root, "ssn_stage1_baselines_vs_stage2_snr_all.pt")
