#!/usr/bin/env python3
"""
compare_datadriven_vs_multisubarray_covariance.py
...
Example:
python compare_datadriven_vs_multisubarray_covariance.py \
  ...
  --multi_subarray_checkpoint_path multi_subarray/latest.pth \
  --batch_size 1024 \
  --n_mc 200 \
  --stddev_spacing 1.0 \
...
def main(argv: Optional[List[str]] = None) -> List[Dict[str, Any]]:
    parser = argparse.ArgumentParser(
        "Compare DataDrivenComplexNet regular / MC-Dropout / CP covariance against MultiSubarraysModel covariance"
    )
    ...
    print("[5/5] MultiSubarraysModel deterministic covariance on test set...")
    ...
    tensors["multi_subarray_model"] = ms_test
    rows.append(covariance_metrics(
