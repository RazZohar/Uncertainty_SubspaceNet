...
Stage-1 / best_base_accuracy.pth is the base estimator for all baselines:
  ...
  3) Absolute-residual source-wise split conformal prediction:
     - uses cp_calibration_dataset.pt, which was split from dataset.pt before
       any model fitting or checkpoint selection;
     - evaluates every method on the entirely separate test_dataset.pt;
...
No training and no dataset generation are performed.  The calibration and test
files are supplied explicitly and must be disjoint.
"""
...
def write_outputs(
    ...
    csv_path = os.path.join(out_dir, "ssn_stage1_absolute_cp_external_calibration_vs_stage2_method_comparison.csv")
    fieldnames: List[str] = []
    for row in rows:
        for key in row.keys():
    ...
    json_path = os.path.join(out_dir, "ssn_stage1_absolute_cp_external_calibration_vs_stage2_method_comparison.json")
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(_jsonify(rows), f, indent=2)

    pt_path = os.path.join(out_dir, "ssn_stage1_absolute_cp_external_calibration_vs_stage2_method_tensors.pt")
    torch.save(
        {
            key: {
...
def main(argv: Optional[List[str]] = None) -> List[Dict[str, Any]]:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Independent CP calibration dataset created from the 20% held-out portion "
            "of the original training dataset. It must not have been used for model fitting, "
            "validation, checkpoint selection, or hyperparameter tuning."
        ),
    )
    parser.add_argument(
        "--test_dataset_path",
        required=True,
        help="New independently generated test dataset used for every reported metric.",
    )
    parser.add_argument(
        "--cp_split_metadata_path",
        default=None,
        help="Optional strict_cp_split_metadata.json file copied into the output manifest.",
    )
    ...
    cal_ds = torch.load(args.cp_calibration_dataset_path, weights_only=False)
    test_ds = torch.load(args.test_dataset_path, weights_only=False)
    _enable_graph_features(cal_ds)
    _enable_graph_features(test_ds)

    n_cal = len(cal_ds)
    n_test = len(test_ds)
    if n_cal < 1 or n_test < 1:
        raise ValueError("Both CP calibration and test datasets must contain at least one scene.")

    progress(
        f"Strict CP data split: calibration={n_cal} scenes from "
        f"{os.path.abspath(args.cp_calibration_dataset_path)}; "
        f"final test={n_test} scenes from {os.path.abspath(args.test_dataset_path)}."
    )
    ...
    progress("[4/5] Strict absolute-residual source-wise CP: q from external calibration dataset; no Stage-1 covariance used...")
    ...
    cp_row["final_evaluation_num_scenes"] = int(n_test)
    cp_row["cp_calibration_dataset_path"] = os.path.abspath(args.cp_calibration_dataset_path)
    cp_row["cp_calibration_source"] = "external 20% split of dataset.pt, unused during model fitting"
    cp_row["test_dataset_path"] = os.path.abspath(args.test_dataset_path)
    ...
    for row in rows:
        ...
        row["final_evaluation_num_scenes"] = int(n_test)
        row["cp_calibration_dataset_path"] = os.path.abspath(args.cp_calibration_dataset_path)
        row["test_dataset_path"] = os.path.abspath(args.test_dataset_path)
    ...
    split_metadata = {
        ...
        "cp_split_metadata_path": os.path.abspath(args.cp_split_metadata_path)
            if args.cp_split_metadata_path else None,
        "note": (
            "q_alpha,k is estimated only from cp_calibration_dataset.pt. All reported "
            "method metrics are evaluated on the full separate test_dataset.pt. CP source "
            "identity is the ascending DoA order."
        ),
    }
    if args.cp_split_metadata_path:
        with open(args.cp_split_metadata_path, "r", encoding="utf-8") as f:
            split_metadata["training_split_metadata"] = json.load(f)
    with open(os.path.join(args.out_dir, "strict_cp_data_split_metadata.json"), "w", encoding="utf-8") as f:
    ...
    csv_path = os.path.join(args.out_dir, "ssn_stage1_absolute_cp_external_calibration_vs_stage2_comparison.csv")
    fieldnames: List[str] = []
    for row in rows:
        for key in row.keys():
            if key not in fieldnames:
    ...
    json_path = os.path.join(args.out_dir, "ssn_stage1_absolute_cp_external_calibration_vs_stage2_comparison.json")
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(_jsonify(rows), f, indent=2)
