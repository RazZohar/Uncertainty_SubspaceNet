...
def esprit_overlapped(Rhat: np.ndarray, d_sources: int, shift: int = 1):
    ...
    idx = np.argsort(evals)[::-1][:d_sources]
    E_s = evecs[:, idx]
...
def bias_mse_var_with_perm(doa_hat_deg_trials: np.ndarray,
    ...
    mse_deg2 = (errs**2).mean(axis=0)                # MSE (deg²)
    # Unbiased sample variance around the (signed) mean:
    # s^2 = (T/(T-1)) * (MSE - bias^2)
    var_unbiased_deg2 = (T/(T-1)) * (mse_deg2 - bias_deg**2)
...
def monte_carlo_compare_eq58_wrapped_no_perm(
    doas_deg, M=8, N=6000, fc=3.5e9, snr_db=-10.0,
...
if __name__ == "__main__":
    ...
    for snr in snrs:
        summary = monte_carlo_compare_eq58_wrapped_no_perm(
            doas_deg=doas,
            M=8, N=TRIALS, fc=3.5e9, snr_db=snr,
            trials=3000,
    ...
    try:
        ...
        for i in range(K):
            ax.plot(snrs, Y[i, :], linestyle='--', marker='o', label=f"Mode {i} Predicted")
            ax.plot(snrs, emprical_var_vs_snr[i, :], linestyle='--', marker='*', label=f"Mode {i} Empirical")

        # --- title + legend (legend sits below the title) ---
        ax.set_title(f"ESPRIT: Predicted vs Empirical DOA Variance vs SNR\n(Δ=λ/2, M=8, N={TRIALS})",
     ...
     borderpad=1.0)
        ...
        for i in range(K):
            axins.plot(snrs, Y[i, :], linestyle='--', marker='o')
            axins.plot(snrs, emprical_var_vs_snr[i, :], linestyle='--', marker='*')
        ...
        ys_inset = np.concatenate([
            Y[:, mask].ravel(),
            emprical_var_vs_snr[:, mask].ravel()
        ]) if np.any(mask) else np.concatenate([Y.ravel(), emprical_var_vs_snr.ravel()])
